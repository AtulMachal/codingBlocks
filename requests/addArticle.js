...
router.post("/api/addArticle", (req, res) => {
    ...
    conn.query(`SELECT * FROM articledetails,approvedby WHERE articledetails.emailHeadingdate = approvedby.emailHeadingDate AND heading="${heading}"`,(err,results)=>{
        if(err){
            console.log(err)
        }else if(results.length == 0){
            if(!req.files){
                conn.query(`INSERT INTO articledetails VALUES("${email}","${heading}","${content1}",${null},"${content2}","${topic}","pending","new","${date}","${email+heading+time}")`,(err,results)=>{
                ...
                if (file.mimetype == "image/jpeg" || file.mimetype == "image/png" || file.mimetype == "image/gif"){
                    ...
                    conn.query(`INSERT INTO articledetails VALUES("${email}","${heading}","${content1}","${email+time+file.name}","${content2}","${topic}","pending","new","${date}","${email+heading+time}")`,(err,results)=>{
                        if(err){
                            console.log(err)
                        }
                        else{
            ...
            res.sendFile(path.join(__dirname+'/exist.html'));
        }
    });
